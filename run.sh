...
python -u -m torch.distributed.launch --master_port 22577 --nproc_per_node=${node_num} train.py  --data ${imagenet_path} --model  ${model}  --lr 4e-3   --output ./output/${model}_${batch_size}x${node_num}   >./${model}_${batch_size}x${node_num}.log 2>&1 &
